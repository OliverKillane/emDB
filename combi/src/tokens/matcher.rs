...
impl <const PEEK: bool, F> Combi for Matcher<PEEK, F> where F: Fn(&TokenTree) -> bool, {
    type Suc = bool;
    ...
    #[inline(always)]
    fn comp(&self, mut input: Self::Inp) -> (Self::Out, CombiResult<Self::Suc, Self::Con, Self::Err>) {
        ...
        if PEEK {
            if let Some(tk) = input.peek_next() {
                ...
                let span = input.cur_span().clone();
                (input, CombiResult::Err(TokenDiagnostic::from(Diagnostic::spanned(
                    span,
                    Level::Error, "Unexpected end of input".to_owned()),
                )))
            }
        } else {
            if let Some(tk) = input.next() {
                let res = CombiResult::Suc((self.match_fn)(&tk));
                (input, res)
            } else {
                let span = input.cur_span().clone();
