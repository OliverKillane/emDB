...
def process_benchmark(data: Any) -> Benchmark | None:
    ...
    try:
        return Benchmark({name: { int(param) : toStatRange(stats["time"])  for (param, stats) in values.items() } for (name, values) in data.items()}) 
...
def plot_line(title: str, param_name: str, bench: Benchmark):
    ...
    for i, (name, values) in enumerate(data.items()):
        ...
        mean_values, min_values, max_values = zip(*[(stat.mean, stat.fastest, stat.slowest) for stat in stats])
...
def plot_speedup(title: str, param_val: int, bench: Benchmark):
    ...
    for i, (name, values) in enumerate(data_items):
        ...
        plt.fill_betweenx([i - 0.4, i + 0.4], fastest_val, slowest_val, color='lightgray') 
        ...
        plt.text(mean_val, i + 0.4, f' {mean_val:.2f}x', color='red', ha='center', va='bottom')
        plt.text(fastest_val, i, f'{fastest_val:.2f}x ', color='blue', ha='right', va='center')
        plt.text(slowest_val, i, f' {slowest_val:.2f}x', color='green', ha='left', va='center')
        labels.append(name)
