...
pub mod generate_access {
    ...
    pub fn validate_select(
        ...
        let table = lp.get_table(table_id);
        let rf_fields = fields
            .into_iter()
            .map(plan::RecordField::from)
            .collect::<Vec<_>>();
        for field in rf_fields {
            ...
            if !table.columns.contains_key(&field) {
                ...
                errors.push_back(errors::query_select_duplicate_field(
                    original.get_field(),
                    field.get_field(),
                ));
            } else {
        ...
        for field in unique_fields {
            let scalar_type =
                lp.scalar_types
                    .insert(plan::ConcRef::Conc(plan::ScalarTypeConc::TableGet {
                        table: table_id,
                        field: field.clone(),
                    }));
