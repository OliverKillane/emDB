...
use pulpit_gen::{columns::{AssocVec, PrimaryRetain}, groups::{Field, Group, GroupConfig, Groups, MutImmut}, namer::CodeNamer, operations::update::Update, predicates::Predicate, table::Table, uniques::Unique};
use quote::ToTokens;
...
fn main() {
    ...
    let table = Table { 
        groups: GroupConfig { 
            primary: Group{ 
                ...
                fields: MutImmut { 
                    imm_fields: vec![
                        Field { name: b.clone(), ty: quote!{usize}.into() },
                    ], 
                    mut_fields: vec![
                        Field { name: a.clone(), ty: quote!{i32}.into() },
                        Field { name: c.clone(), ty: quote!{Option<String>}.into() },
                    ]
                } 
            }, assoc: vec![
                Group{ 
                    ...
                    fields: MutImmut { 
                        imm_fields: vec![
                            Field { name: d.clone(), ty: quote!{char}.into() }
                            ], 
                            mut_fields: vec![
                            Field { name: e.clone(), ty: quote!{String}.into() },
                        ] 
                    } 
                }
            ] }.into(), 
        uniques: HashMap::from(
            [
                (
                    e.clone(),
                    Unique { alias: Ident::new("e_unique", Span::call_site())}
                ),
                (
                    a.clone(),
                    Unique { alias: Ident::new("a_unique", Span::call_site())}
                )
            ]
        ), 
        predicates: vec![
            Predicate { alias: Ident::new("check_b", Span::call_site()), tokens: quote!{*b < 1045}.into() },
            Predicate { alias: Ident::new("check_e_len", Span::call_site()), tokens: quote!{e.len() > *b}.into()},
        ], 
        updates: vec![
            Update{ 
    ...
    fs::write("../pulpit/tests/output.rs", unparse(&parse2(tks.into_token_stream()).unwrap())).unwrap();
}
