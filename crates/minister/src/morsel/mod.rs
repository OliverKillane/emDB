...
use datum::Datum;
use rayon::{iter::{IntoParallelIterator}, result, scope};
use splitter::{EvenSplit, Splitter};
use morsel::Morsel;
...
impl MorselOps for MorselParallel {
    ...
    fn export_stream<Data>(stream:stream!(Data)) -> impl Iterator<Item = Data>where Data:Send+Sync {
        Self::export_buffer(stream).into_iter()
    }

    fn export_buffer<Data>(mut stream:stream!(Data)) -> Vec<Data>where Data:Send+Sync {
        let splitter = Split::create();
        match stream.runtime_cardinality() {
            statistics::Cardinality::Exact(i) => {
                // NOTE: exact size of end buffer known, so can pre-allocate and split.

                let mut results = Vec::with_capacity(i);
                for _ in 0..i {
                    results.push(Datum::new_empty())
                }
                let result_spans = splitter.split_slice_mut(&mut results);
                
                // TODO: Given we know the exact size of the *whole* result, can we assume it is evenly split between buffers?
                //       Feel like this relies on the fact that only Map and union retains the exact cardinality.
                scope(|s| {
                    for (w, span) in stream.work(&splitter).zip(result_spans) {
                        s.spawn(|_| {
                            for (d, slot) in w.zip(span.iter_mut()) {
                                slot.put(d, datum::DatumState::Uninitialised)
                            }
                        });
                    }
                });

                // NOTE: in-place collect means this becomes zero cost on release
                results.into_iter().map(|mut d| d.get()).collect()
            },
            statistics::Cardinality::Bound(_) | statistics::Cardinality::Unknown => {
                // NOTE: unknown size, so must collect into a buffer and then split
                stream.work(&splitter).map(|i| i.collect::<Vec<_>>()).reduce(|mut a,mut b| {
                    a.append(&mut b);
                    a
